// src/App.tsx - UPDATED: Added UsePage
...
function AppContent() {
  ...
  const [authMode, setAuthMode] = useState<'login' | 'register' | 'usage'>('login');
  ...
  // Not authenticated - show login/register/usage
  if (!user) {
    if (authMode === 'register') {
      return <RegisterPage onNavigateToLogin={() => setAuthMode('login')} />;
    }
    if (authMode === 'usage') {
      return <UsePage onNavigateToLogin={() => setAuthMode('login')} />;
    }
    return (
      <LoginPage 
        onNavigateToRegister={() => setAuthMode('register')}
        onNavigateToUsage={() => setAuthMode('usage')}
      />
    );
  }
